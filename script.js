...
const questionsPool = [
    { question: "Wer schoss den Siegtreffer für den FC Bayern im Champions-League-Finale 2013?", image: "https://upload.wikimedia.org/wikipedia/commons/thumb/c/cf/Arjen_Robben_in_2013.jpg/320px-Arjen_Robben_in_2013.jpg", answers: [{ text: "Thomas Müller", correct: false }, { text: "Franck Ribéry", correct: false }, { text: "Arjen Robben", correct: true }, { text: "Mario Mandžukić", correct: false }], explanation: "In der 89. Minute spitzelte Arjen Robben den Ball an Roman Weidenfeller vorbei ins Netz und erlöste ganz München.", difficulty: 'leicht' },
    { question: "Welcher Trainer führte die Bayern 2013 zum ersten Triple der Vereinsgeschichte?", image: "https://upload.wikimedia.org/wikipedia/commons/thumb/e/e0/Jupp_Heynckes.jpg/320px-Jupp_Heynckes.jpg", answers: [{ text: "Pep Guardiola", correct: false }, { text: "Jupp Heynckes", correct: true }, { text: "Louis van Gaal", correct: false }, { text: "Carlo Ancelotti", correct: false }], explanation: "Jupp Heynckes krönte seine Karriere mit dem Triple aus Meisterschaft, DFB-Pokal und Champions League, bevor Pep Guardiola übernahm.", difficulty: 'leicht' },
...
function startQuiz() {
    ...
    const filteredPool = questionsPool.filter(q => q.difficulty === selectedDifficulty);

    if (filteredPool.length < 10) {
        questionElement.innerText = `Für die Schwierigkeit "${selectedDifficulty}" sind leider noch nicht genügend Fragen vorhanden. Bitte wähle eine andere.`;
        resetState();
        answerButtonsElement.innerHTML = ""; // Ensure no buttons are shown
...
function getLeaderboardKey() {
    ...
    return `bayernLeaderboard_${difficulty}`;
}

function renderLeaderboard() {
    const leaderboard = JSON.parse(localStorage.getItem(getLeaderboardKey())) || [];
